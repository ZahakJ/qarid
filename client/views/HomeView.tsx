...
import { AnthologyStrip } from "./AnthologyView.tsx"
...
export function HomeView() {
  // On a phone the app bar already carries «قريض», so the hero wordmark and its
  // lede are two screens' worth of masthead saying the name twice. The page
  // leads with بيت اليوم instead, and the corpus counts — the only fact the
  // lede carried — become the one small line under its card (the same
  // `.home__foot` the web draws at the very bottom, moved).
  ...
  return (
    <div className="view view--centred home">
      {native ? null : (
        <header className="home__mast">
          ...
          <p className="home__lede">
            {meta
              ? `${formatPoems(meta.counts.poems)}، و${formatBaits(meta.counts.baits)}، ${lamPrefix(formatPoets(meta.counts.poets))} — تُقرأ وتُساجَل.`
      ...
      {/* ── المختارات المنظومة ────────────────────────────────────────── */}
      {/* Under بيت اليوم and above the search box on purpose: a reader who
          arrived with a line in their head reaches for the omnibox and never
          scrolls, but a reader who arrived with nothing needs to be shown a
          door before they are handed a text field. */}
      <AnthologyStrip />

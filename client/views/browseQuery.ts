...
export function appliedChips(q: BrowseQuery, meta: MetaResponse | null): AppliedChip[] {
  ...
  // ONE name for one control: the rail heading and the empty-state hint say
  // «الرويّ», so the chip for the very same selection says «رويّ اللام» — not
  // «قافية اللام», which read as a different filter two screens apart.
  if (q.rawiyy) out.push({ key: "rawiyy", value: q.rawiyy, kind: "letter", label: `رويّ ${letterName(q.rawiyy)}` })
  if (q.letter)

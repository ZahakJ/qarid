...
import { PALETTE_EMPTY } from "../components/palette.ts"
...
export function SearchView({ q, page }: { q: string; page: number }) {
  ...
  /** the query without its عبارة quotes — what the two widening doors act on */
  const bare = unquote(q)
  ...
  return (
    <div className="view search-view">
              ...
              <>
                ...
                <span>
                  {/* ONLY the number gets the LTR isolate. Wrapping the whole
                      ...
                      reordered and the reader got «الثانية ألف من 6.1».
                      The unit is WRITTEN, not transliterated: «مِلّي» was
                      "milli" glued to ثانية, the one unit in the app that was
                      not Arabic. */}
                  <span className="search-ms">{res.ms.toFixed(res.ms < 10 ? 1 : 0)}</span> من ألف الثانية
                </span>
              </>
            ) : null}
          </p>
      ...
      {error ? (
        ...
        /* NOT A DEAD END. The phone's own full-screen search — the same
           `usePaletteSearch` behind it — already answers this with «جرّب كلمةً
           أقصر، أو نجمةً في آخرها» and a way out; #/browse's zero state offers
           the facets to drop. The route a reader actually deep-links and
           shares was the bare one: a sentence and nothing to press.

           Three doors, in the order a reader would try them: widen the query
           («أيّ كلمة» is one control away and this screen already names the
           mode in prose), open it on the prefix (the one piece of syntax the
           app honours), and leave for التصفح. */
        <EmptyState flavor="search-none">
          <p className="search-count">
            لا شيء يطابق «<bdi>{q}</bdi>»{anyFilter ? " على هذه القيود" : ""}.
          </p>
          <p className="empty__note">{PALETTE_EMPTY}</p>
          <div className="search-none__acts">
            {anyFilter ? (
            ...
            {bare && !bare.endsWith("*") ? (
              <button
                type="button"
                className="btn"
                onClick={() => navigate({ view: "search", q: `${bare}*`, page: 1 }, true)}
              >
                ابحث عمّا بدأ بها
              </button>
            ) : null}
            <a className="btn btn--ghost" href={routeHash({ view: "browse", query: {} })}>
              تصفّح الديوان
            </a>
          </div>
        </EmptyState>

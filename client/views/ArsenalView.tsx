          ...
          <ul className="weakrow-list">
            {weak.map((s) => (
              <li className="weakrow" key={s.letter}>
                ...
                {/* A LINK, like the panel's own door below — five buttons
                    calling navigate() could not be opened in a new tab, and
                    their accessible name was the bare word with no letter in
                    it, so a screen reader heard the same control five times.
                    The visible label stays short for the row; the letter is in
                    the accessible name. */}
                <a
                  className="btn btn--ghost btn--sm"
                  href={routeHash({ view: "train-drill", letter: s.letter })}
                  aria-label={`ذاكِر حرف ${s.letter}`}
                >
                  ذاكِر
                </a>
              </li>
...
function LetterDetail({ stat }: { stat: LetterStat }) {
  return (
    <section className="letter-detail" aria-live="polite">
      ...
      {/* Three sentences, not one with a hole in it. «ينقصك 3 من 5» told the
          reader neither what three of five ARE nor, at zero supply, anything at
          all («ينقصك 8 من 8» reads as a broken counter). The معدود goes through
          formatBaits like its sibling branch. */}
      <p className="letter-detail__note">
        {stat.supply >= SUPPLY_TARGET
          ? `هذا الحرف مؤمَّن — في ترسانتك منه ${formatBaits(stat.supply)}.`
          : stat.supply === 0
            ? `لم تحفظ من هذا الحرف شيئًا — ${formatBaits(SUPPLY_TARGET)} تُؤمّنه.`
            : `ينقصك ${formatBaits(SUPPLY_TARGET - stat.supply)} حتى يُؤمَّن هذا الحرف — عندك ${formatCount(stat.supply)} من ${formatCount(SUPPLY_TARGET)}.`}
      </p>
      <div className="letter-detail__acts">
        <a className="btn btn--primary btn--sm" href={routeHash({ view: "train-drill", letter: stat.letter })}>
          ذاكِر حرف {stat.letter}
        </a>

 ...
 *
 * ON A PHONE the rail is a BOTTOM SHEET. Below 1100px it was already a
 * disclosure that unfolded ABOVE the results — which is a rail pretending to
 * still be a column: opening it pushed the قصائد off the screen, and choosing a
 * قيد left the reader looking at five accordions instead of at what they had
 * just asked for. The same five accordions in a sheet come up over the results,
 * dismiss with a tap, and close on «أظهر النتائج». Same DOM, same state, same
 * URL; only the surface differs, and only on a coarse pointer.
 */
...
import { browseQueryString, navigate, routeHash, type BrowseQuery, type BrowseSort } from "../router.ts"
...
export function BrowseView({ query }: { query: BrowseQuery }) {
  ...
  /**
   * The five accordions, written once. They are the browse column on a desktop
   * and the contents of a sheet on a phone, and they must be the SAME nodes:
   * two copies of a facet rail is two places to add the sixth facet to.
   */
  const rail = (
    <aside className="facet-rail" aria-label="القيود">
      ...
      <Accordion label={FACET_LABEL.rawiyy} open={Boolean(query.rawiyy)}>
        <LetterGrid label="الرويّ" counts={facets?.rhymes ?? null} active={query.rawiyy} onPick={(l) => pick("rawiyy")(l)} />
        {/* A quiet door, and it belongs HERE rather than in the toolbar: the
            reader who has just tapped a روي well is the one reader in the app
            who might be hunting a قافية rather than browsing one. It carries
            the letter they already chose, so the tool opens on their answer
            instead of on its own empty state. */}
        <a className="facet__aside" href={routeHash({ view: "qafiya", query: { rawiyy: query.rawiyy } })}>
          تبحث عن قافية؟ ←
        </a>
      </Accordion>
  ...
  return (
    <div className="view browse-view">
      ...
      <div className="browse-shell" data-rail-open={!native && railOpen ? "1" : undefined}>
        {/* ── the facet rail ───────────────────────────────────────────── */}
        {native ? null : rail}
        ...
        <div className="browse-main">
          {error ? (
            ...
            // ONE noun for one zero. The count line above says «لا أبيات على
            // هذه القيود» whenever a رويّ makes this بيت-mode, while the empty
            // state three lines under it said «لا قصيدة بهذه القيود» — two
            // words for the same nothing. Both come off `baytMode` now.
            <EmptyState flavor="facets-zero" title={baytMode ? "لا أبيات بهذه القيود" : "لا قصيدة بهذه القيود"}>
      ...
      {/* The same rail, on a phone, over the results instead of above them.
          «أظهر النتائج» is the sheet's own way out and it says what it is for —
          every chip has already applied itself, so the button dismisses rather
          than confirms, and the count in it is the answer to what was asked. */}
      {native && railOpen ? (
        <Sheet
          title="القيود"
          note="اختر عصرًا أو بحرًا أو غرضًا أو رويًّا — والنتائج تتبدّل تحتك."
          onClose={() => setRailOpen(false)}
          className="sheet--facets"
          footer={
            <div className="sheet__acts">
              <button type="button" className="btn btn--primary" onClick={() => setRailOpen(false)}>
                أظهر النتائج
                <span className="sheet__act-n">{baytMode ? formatBaits(total) : formatPoems(total)}</span>
              </button>
              {chips.length > 0 ? (
                <button
                  type="button"

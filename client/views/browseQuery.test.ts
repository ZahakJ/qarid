...
describe("appliedChips", () => {
  ...
  it("names each letter facet the way its own rail heading does — رويّ vs مطلع", () => {
    const chips = appliedChips({ rawiyy: "م", letter: "ا" }, meta)
    // The rail heading says «الرويّ», so the chip for the same selection says
    // «رويّ الميم» — «قافية الميم» read as a different control two screens apart.
    expect(chips.map((c) => c.label)).toEqual([`رويّ ${letterName("م")}`, `مطلع بـ${letterName("ا")}`])

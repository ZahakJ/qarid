...
import { GRADE_LABEL, MAX_NEW_PER_DAY, buildQueue, seedOf, type CardSeed } from "../training/schedule.ts"
...
export function DrillView({ letter }: { letter?: string }) {
  ...
  useEffect(() => {
    ...
    async function plan() {
      ...
      const now = Date.now()
      // `letter` restricts BOTH halves of the plan, not just which new أبيات
      // are asked for — see the note on `QueueInput.letter`. Without it, the
      // cards the previous letter's session introduced are due now and lead
      // this queue, so a second «ذاكِر» drilled the first letter again.
  ...
  const head = (
    <header className="view__head">
      ...
      <p className="view__lede">
        {letter ? (
          <>
            مذاكرةٌ على حرف <span className="drill-letter">{letter}</span> — {LETTER_NAMES[letter as HijaiLetter]}.
          </>
        ) : (
    ...
    return (
      <div className="view drill-view">
        ...
        {done ? (
          ...
          // A letter session that comes up empty says WHY it is empty: the
          // deck owes nothing on this letter and the day's ration of new أبيات
          // is spent. «لا بطاقة مستحقّة اليوم» alone, on a door that named a
          // letter, reads as the door being broken.
          <EmptyState
            flavor="no-due"
            title={error ?? (letter ? `لا بطاقة على حرف ${letter} اليوم` : "لا بطاقة مستحقّة اليوم")}
          >
            {letter && !error ? (
              <p className="empty__note">أخذتَ نصيبك من الأبيات الجديدة اليوم. عُد إليه غدًا، أو ذاكِر ما استُحقّ.</p>
            ) : null}
            <div className="drill-done__acts">
              ...
              {letter ? (
                <a className="btn btn--ghost" href={routeHash({ view: "train-drill" })}>
                  ذاكِر ما استُحقّ
                </a>
              ) : (
              ...
              meta={
                <span className="drill-reveal">
                  ...
                  {card.firstLetter ? (
                    ...
                    >
                      {inArsenal ? "صار في ترسانتي" : "هذا في ترسانتي"}
                    </button>
                  ) : null}
                </span>
              }

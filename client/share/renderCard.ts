...
export function renderCard(bayt: CardBayt, shape: CardShape = "wide"): HTMLCanvasElement {
  ...
  // ── the mark ───────────────────────────────────────────────────────────
  ctx.fillStyle = GOLD
  ctx.font = `400 ${shape === "wide" ? 54 : 60}px "Aref Ruqaa", serif`
  ctx.textAlign = "right"
  ctx.fillText("قريض", w - Math.round(w * 0.062), h - Math.round(h * 0.062))
  ctx.fillStyle = "rgba(92,124,200,.9)"
  ctx.font = `400 ${shape === "wide" ? 18 : 20}px "IBM Plex Sans Arabic", sans-serif`
  ctx.textAlign = "left"
  ctx.direction = "ltr"
  ctx.fillText("qarid.avicenna.space", Math.round(w * 0.062), h - Math.round(h * 0.062))
  void LAPIS

 ...
 *
 * `network` is one of them, and it was the bug. It rode `softReject`'s 1.2 s
 * like a wrong letter — but it is not a soft rejection, it is a DEMAND: four
 * lines of Arabic and an «أعد المحاولة» button. Sampled at 300/700/1200/2000 ms
 * it was present through 1200 and gone by 2000, so three seconds after a dead
 * request the screen showed nothing at all and the reader tapped «أجب» into
 * silence. It holds until they retry or type something else, and the clock is
 * frozen outright while it stands (`pauseIsPlayers` in machine.ts).
 */
function needsAnswer(r: Rejection | null): boolean {
  return r?.kind === "near_miss" || r?.kind === "network"
}
...
/**
 * The ديوان pool on the wire, or nothing at all.
 *
 * `[]` and «absent» are the same thing to the server, and both mean «the whole
 * corpus» — so an ordinary duel simply does not carry the field, and a duel
 * inside a shelf carries every id of it on every request. The pool lives on the
 * persisted config, so a reload keeps playing the same ديوان.
 */
function poolOf(s: DuelState): { poolBaitIds?: number[] } {
  return s.config.album ? { poolBaitIds: s.config.album.baitIds } : {}
}
...
async function deal(s: DuelState): Promise<void> {
  ...
  try {
    ...
    if (res.ok) dispatch({ type: "DEALT", served: served(res), now: Date.now() })
      ...
      dispatch({
        type: "DEAL_FAILED",
        // Inside a ديوان there are no «قيود» to loosen — the shelf is the
        // whole pool — so the sentence that names them would be advice the
        // reader cannot act on.
        message: s.config.album
          ? `لم أجد في «${s.config.album.title}» بيتًا أبدأ به`
          : "لا أبيات بهذه القيود — خفّف القيود ثم أعد",
        now: Date.now(),
      })
  } catch (e) {
    if (mine !== epoch) return
...
export async function buyHint(kind: HintKind): Promise<void> {
  ...
  try {
    const res = await gameHint({
      ...
      difficulty: s.config.difficulty,
      filters: s.config.filters,
      // «بدّل الحرف» hands over a fresh OPPONENT بيت, so inside a ديوان it
      // comes off the shelf; the three cheap hints describe a بيت the PLAYER
      // could answer with and are not pooled (shared/schema.ts).
      ...(kind === "switch_letter" ? poolOf(s) : {}),
      seed,
    })
    if (!res.ok) return
    const now = Date.now()

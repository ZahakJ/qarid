...
export type AssistRailProps = {
  ...
  letter: string | null
  /** a second accepted opening letter (rhyme mode's وصل), when there is one */
  also?: string | null
...
export function AssistRail({ items, loading, letter, also = null, hasQuery, onPick }: AssistRailProps) {
      ...
      <p className="assist__head">
        ...
        <span className="assist__lede">
          {letter === null ? (
            ...
            <>
              أبيات من الديوان تبدأ بحرف <span className="assist__letter">{letter}</span>
              {also !== null && also !== letter ? (
                <>
                  {" "}
                  أو <span className="assist__letter">{also}</span>
                </>
              ) : null}{" "}
              — انقر البيت ليُكتب لك
            </>
          )}
        </span>

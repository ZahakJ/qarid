...
function FriendDialog({ onClose }: { onClose: () => void }) {
  ...
  // and on a phone `Sheet` owns that, along with the Tab trap and the restore.
  useEffect(() => {
    if (native) return
  ...
  useEffect(() => {
    if (!native) dialogRef.current?.focus()
  }, [native])
  ...
  const create = async () => {
    ...
    try {
      ...
      setBusy(false)
      // A dead session closes this surface and opens دخول with the reason on
      // it — the sheet used to print «ادخل بحسابك أوّلًا» in its scrolling body
      // under the sticky bar, with no way to sign in and a signed-in avatar
      // still on the app bar.
  ...
  // The four decisions, written once: the same nodes fill the desktop's card
  ...
  /* primary FIRST, so it lands on the reading edge — `.auth__actions` and the
     walkthrough's footer are laid out the same way. */
  const acts = (
    <>
  ...
  if (native) {
    return (
      <Sheet
        title="غرفة مساجلة"
        note="اختر المطلع وقاعدة السلسلة، ثم افتح الغرفة وابعث رابطها."
        onClose={onClose}
        className="sheet--friend"
        footer={<div className="sheet__acts">{acts}</div>}
      >
        {body}
      </Sheet>
    )
  ...
  return (
    <Overlay onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      ...
      >
        ...
        <footer className="friend-dialog__foot">{acts}</footer>
      </div>
    </Overlay>
  )
}

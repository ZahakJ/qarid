...
const ON = { enabled: true, letter: "ل" }
...
describe("assistPlan — one request per pause, and not one more", () => {
  it("asks for nothing at all in a normal duel", () => {
    ...
    expect(assistPlan(newAssistMemo(), { enabled: false, letter: "ل", draft: "لا خيل" })).toEqual({ kind: "idle" })
  })

  it("asks for nothing before there is a letter to chain on", () => {
    expect(assistPlan(newAssistMemo(), { enabled: true, letter: null, draft: "لا خيل" })).toEqual({ kind: "idle" })
  })
  ...
  it("asks for nothing under the minimum", () => {
    ...
    expect(assistPlan(newAssistMemo(), { ...ON, draft: "لا" })).toEqual({ kind: "fetch", letter: "ل", needle: "لا" })
  })
  ...
  it("never re-asks about a prefix that already answered with nothing", () => {
    ...
    expect(assistPlan(memo, { ...ON, draft: "لا" })).toEqual({ kind: "fetch", letter: "ل", needle: "لا" })
    expect(assistPlan(memo, { enabled: true, letter: "ا", draft: "لا خيل عندك" })).toEqual({
      kind: "fetch",
      letter: "ا",
      needle: "لا خيل عندك",
    })
  ...
  it("keeps the two letters' answers apart", () => {
    ...
    expect(assistPlan(memo, { enabled: true, letter: "ا", draft: "لا تحسب" })).toEqual({
      kind: "fetch",
      letter: "ا",
      needle: "لا تحسب",
    })

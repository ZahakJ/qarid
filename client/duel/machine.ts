...
export type Rejection =
  ...
  /** «ليس من هذا الديوان» — a room played inside a ديوان, and only there */
  | { kind: "not_in_album"; bait: BaitDto; albumTitle: string }
 ...
 *
 * With ONE exception, and it is the same rule read correctly: a `network` card
 * is not a card the player chose to leave up. The request failed, «لا تُحتسب»,
 * and the card's own button is «أعد المحاولة» — so the span belongs to the
 * server exactly as `verifying` does. Measured before this: the clock ran on
 * through a dead /api/game/verify (0:36 and falling) while the player pressed
 * «أجب» into silence, and the turn was lost to an outage.
 */
function pauseIsPlayers(s: DuelState): boolean {
  if (s.rejection?.kind === "network") return false
  ...
  switch (a.type) {
    ...
    case "VERIFIED": {
      ...
      switch (r.reason) {
        ...
        case "not_in_album":
          // A room's refusal, never the solo duel's: the pool narrows what the
          // OPPONENT recites and nothing about what the player may answer. The
          // branch exists because one verify union serves both, and because a
          // rejection nobody handled would freeze the screen on `verifying`.
          return softReject(s, { kind: "not_in_album", bait: r.bait, albumTitle: r.albumTitle }, "not_in_album", a.now)

...
/**
 * NEWEST AT THE BOTTOM → follow it. Two scrollers, and which one is in play is
 * a fact the VIEW knows, not one this can measure.
 *
 * On the web the page scrolls, and `scrollIntoView` on an end sentinel is what
 * it has always been — untouched here, down to the single call, because
 * App.tsx's own «a new page starts at its top» effect runs after this one and
 * settles the argument between them.
 *
 * On the phone's game screen the LOG is the scroller (client/styles/phone.css),
 * `scrollIntoView` is the wrong tool (it walks ancestors, and the ancestor is an
 * `overflow: hidden` column that must not move), and the box does not yet KNOW
 * it is a scroller: `body[data-immersive]` is written by a layout effect one
 * commit later, and React flushes this passive effect before that commit.
 * Measured without the retry: a duel resumed on a phone opened on the FIRST بيت
 * of the transcript, with the newest one 468px below the fold — a chat that
 * opens on its oldest message. Hence `docked`, and hence the second pass on the
 * next frame.
 */
export function useFollowNewest(
  ...
  deps: readonly unknown[],
  {
    ...
    reduced,
    endRef,
  }: { enabled?: boolean; docked: boolean; reduced: boolean; endRef?: { current: HTMLElement | null } },
): void {
  useEffect(() => {
    if (!enabled) return
    const behavior = reduced ? ("auto" as const) : ("smooth" as const)
    if (!docked) {
      const end = endRef?.current ?? ref.current?.lastElementChild
      ;(end as HTMLElement | null | undefined)?.scrollIntoView({ behavior, block: "end" })
      return
    }
    const follow = () => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- `deps` IS the identity of "there is something new"
  }, [...deps, enabled, docked, reduced])
}
...
export type ExchangeLogProps = {
  ...
  /**
   * The phone's game screen: this log is its own scroller, not the page's
   * (client/styles/phone.css). Only the view knows — see `useFollowNewest`.
   */
  docked?: boolean
  /**
...
export function ExchangeLog({
  ...
  // The summary is read from the top; only a running مساجلة follows its newest.
  useFollowNewest(listRef, [exchanges.length, revealIndex], {
  ...
  return (
    <ol className="exchange-log" data-variant={variant} ref={listRef}>
      {exchanges.map((ex, i) => {
          ...
          <li className="exchange" key={`${ex.baytKey}:${i}`} data-side={ex.side} data-exchange-key={ex.baytKey}>
            {i === revealIndex ? (
              ...
              <BaytPlate
                ...
                /* The ديوان action rides with the ♥, which the LIVE duel does
                   not offer: mid-مساجلة there is a clock running and the
                   opponent's بيت is a thing to answer, not to file. The summary
                   passes both and gets both. */
                onAlbum={onFavorite ? albumAction(ex) : undefined}

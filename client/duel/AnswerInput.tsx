...
export function AnswerInput({
   ...
   *
   * `scrollHeight` is the CONTENT box plus padding and excludes the border,
   * while `height` on a `border-box` element includes it — so the field has
   * always been two pixels short of what it holds. On the web that is invisible
   * (measured: clientHeight 90 against a scrollHeight of 92, and the tail of a
   * two-line بيت a hair under the edge); in the phone's dock, where the field
   * sits directly on the bottom edge and Amiri's descenders run low, it clips
   * the second line's tail visibly. The border is added back on the phone only,
   * because the desktop's measure is frozen and this is a two-pixel cosmetic.
   */
  useEffect(() => {
    const el = ref.current
    if (!el) return
    el.style.height = "auto"
    const border = native ? el.offsetHeight - el.clientHeight : 0
    el.style.height = `${Math.min(el.scrollHeight + border, 260)}px`
  }, [value, native])
  ...
  // The second accepted opening letter (rhyme mode's وصل leniency — ever only
  // one), so the rail offers أبيات on it too, not just the روي.
  const alsoLetter = alsoAccepted.find((l) => l !== required) ?? null
  const suggestions = useAssist({ enabled: assist, required, also: alsoLetter, draft: value })

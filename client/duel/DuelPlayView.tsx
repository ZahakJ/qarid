...
export function DuelPlayView() {
  ...
  // On a phone the app bar says which ديوان is being played, because the
  // docked game screen has no line to spare for it.
  useChromeTitle(session?.config.album ? `ديوان «${session.config.album.title}»` : undefined)
  ...
  // THE GAME SCREEN (client/store/immersiveStore.ts). On a phone, for as long
  // as there is a مساجلة that has not reached its summary, this screen takes
  // the whole viewport: no tab bar, and the back chevron asks «انسحب؟».
  useImmersive(native && session !== null && phase !== "summary", abandonDuel)
  ...
  const clockTone = ms === null ? undefined : ms <= 2000 ? "danger" : ms <= 5000 ? "warn" : undefined
  ...
  return (
    <div className="view duel-play" data-phase={phase}>
      ...
      {/* THE ديوان, when the opponent is confined to one. It is a promise being
          kept in front of the reader — «he answers only from this shelf» — and
          it is a link, because the next thing anyone wants after a بيت he did
          not know is the ديوان it came from. On a phone it is the app bar's
          title instead (see `useChromeTitle` above): the docked game screen
          gives its rows to the transcript and the composer, and a fifth line of
          furniture there is one the thumb has to scroll past. */}
      {session.config.album && !native ? (
        <p className="duel-scope">
          الخصم لا يُنشد إلا من ديوان{" "}
          <a href={routeHash({ view: "diwan", code: session.config.album.code })}>
            <bdi>«{session.config.album.title}»</bdi>
          </a>
          ، وأنت تُجيب من الديوان كلّه.
        </p>
      ) : null}
      ...
      <section className="duel-desk" aria-label="جوابك">
        ...
        {/* On the phone's game screen the clock is up in the slim HUD; down
            here it would be the one number the answering thumb covers. */}
        {ms !== null && !native ? (
          <p className="duel-clock" data-tone={clockTone}>
            {formatClock(ms)}
          </p>
        ) : null}
        ...
        <AnswerInput
          ...
          placeholder={placeholderFor(session.seed, session.exchanges.length, session.config.tier, native)}
          required={session.required.letter}
          alsoAccepted={session.required.alsoAccepted}

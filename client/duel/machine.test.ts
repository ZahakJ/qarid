...
describe("verifying → rejected (soft: costs nothing)", () => {
  ...
  it("freezes the clock OUTRIGHT while the network card stands — it is the server's pause", () => {
    /**
     * A network card is not a card the player chose to leave up: the request
     * failed, «لا تُحتسب», and the card's own button is «أعد المحاولة». It used
     * to ride the PLAYER's capped pause with every other `rejected` card, so a
     * dead /api/game/verify kept the turn clock running while the reader read
     * four lines of Arabic and reached for the button.
     */
    const v = reduce(awaiting(), { type: "SUBMIT", text: "شيء", now: T0 + 3_000 })
    const rej = reduce(v, { type: "VERIFY_ERROR", message: "تعذّر الاتصال بالخادم", now: T0 + 7_000 })
    const frozen = timeLeft(rej, T0 + 7_000)
    // a full minute later it has not moved a millisecond
    expect(timeLeft(rej, T0 + 67_000)).toBe(frozen)
    // …while a card the player DOES control still runs out its credit
    const near = reduce(v, {
      type: "VERIFIED",
      response: { ok: false, reason: "near_miss", normalized: "ن", suggestion: bait({ id: 777 }), score: 0.5, acceptCost: HINT_COSTS.accept_near_miss },
      now: T0 + 7_000,
    })
    expect(timeLeft(near, T0 + 67_000)!).toBeLessThan(timeLeft(near, T0 + 7_000)!)
  })
})

/**
 * The hover rail that hangs off a بيت: نسخ · بطاقة · ♥ · ديوان · ساجِلني
...
import { CardMark, CopyMark, DiwanMark, DuelMark, HeartMark } from "./icons.tsx"
...
export type BaytActionsProps = {
  ...
  onFavorite?: () => void
  /**
   * أضِف إلى ديوان — the COLLECTION gesture, beside the ♥ rather than instead
   * of it. ♥ is المختارات: local, instant, one list, and it works signed out. A
   * ديوان is named, ordered, shareable and lives on the server. They are two
   * different acts and a reader does both.
   *
   * Undefined for a بيت that cannot be anchored (no عجز, so no `h_full`) —
   * which is exactly the rule below: an action with no handler is not rendered.
   */
  onAlbum?: () => void
    ...
    <div className="bayt-acts" data-layout={layout} role="group" aria-label={`إجراءات البيت${suffix}`}>
      ...
      {onAlbum ? (
        <button
          type="button"
          className="bayt-act"
          title="أضِف إلى ديوان"
          aria-label="أضِف البيت إلى ديوان"
          onClick={onAlbum}
        >
          <DiwanMark />
        </button>

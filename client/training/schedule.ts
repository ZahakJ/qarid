...
import type { Arsenal, BaitDto, CardGrade, TrainingCard } from "../../shared/schema.ts"
...
/**
 * One corpus بيت as a card seed — the ONE spelling of that mapping.
 *
 * The drill makes seeds out of `/api/train/candidates`, and «أضِفه إلى
 * التحفيظ» makes them out of a ديوان's resolved أبيات. Both must produce the
 * same `id` (`baytKey`) for the same بيت, or a بيت imported from a shelf would
 * come back as a second card the next time the drill offered it.
 */
export function seedOf(bait: BaitDto): CardSeed {
  return {
    id: bait.baytKey,
    baitId: bait.id,
    sadr: bait.sadr,
...
export type QueueInput = {
  ...
  maxNew?: number
  /**
   * `#/train/drill?letter=<L>` — a session about ONE letter.
   *
   * Without it, a letter-targeted drill only BIASED which new أبيات were asked
   * for, while the due half was whatever the deck owed today — and the أبيات
   * the previous letter's session had just introduced are due immediately. So
   * tapping «ذاكِر» on ظ and then on ج drilled ظ both times: the heading changed
   * and the card did not (measured on the real deck). A door that names a
   * letter has to open on that letter, so both halves are restricted here.
   *
   * A card the deck has no letter for is never in a letter session — it is not
   * an answer to «ذاكِر حرف ج».
   */
  letter?: string | null
}
...
export function buildQueue(input: QueueInput): QueuePlan {
  const { cards, now, candidates = [], arsenal = {}, newIntroducedToday = 0, letter = null } = input
  ...
  const onLetter = (l: string | null | undefined) => letter === null || l === letter

  const dueCards = Object.values(cards)
    .filter((c) => isDue(c, now) && onLetter(c.firstLetter))
    .sort((a, b) => a.due - b.due || a.addedAt - b.addedAt)
    .slice(0, maxDue)

  const held = heldByLetter(cards)
  const seen = new Set(Object.keys(cards))
  const offered = candidates.filter((c) => !seen.has(c.id) && onLetter(c.firstLetter))

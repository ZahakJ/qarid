...
import { meterRef, poetRef } from "../dto.ts"
import {
  comboByLetter,
  effectiveTailBias,
  firstWordOf,
...
export function gameRoutes(db: Db, _config: Config): Hono {
  ...
  app.get("/pool", (c) => {
    ...
    const body: GamePoolResponse = {
      total: byLetter.reduce((sum, row) => sum + row.count, 0),
      byLetter,
    }
    return c.json(body)
  })
  ...
  app.post("/reply", async (c) => {
    ...
    if (picked === null) {
      // «أفحمتَ الخصم» — the opponent has nothing left on this letter (+500).
      const body: GameReplyResponse = { ok: false, reason: "no_bait", letter: req.letter }
      return c.json(body)
    }
    const body: GameReplyResponse = { ok: true, seed: req.seed ?? null, ...servedBait(picked.row, req.mode) }
    return c.json(body)
  })
  ...
  app.post("/hint", async (c) => {
    ...
    if (req.kind === "switch_letter") {
      const row = switchLetterBait(db, req.letter, req.filters, req.seed)
    ...
    const letter = requiredLetterFor(db, req.baitId, req.letter)
 ...
 */
function requiredLetterFor(db: Db, baitId: number | undefined, letter: string | undefined): string | null {
  if (baitId !== undefined) {
    const row = db.q("SELECT rawiyy, last_letter FROM baits WHERE id = ?").get(baitId) as
      | { rawiyy: string | null; last_letter: string | null }
      | undefined
    if (row !== undefined) {
      const derived = row.rawiyy ?? row.last_letter

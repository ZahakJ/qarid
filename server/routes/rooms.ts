 ...
 *
 *   POST /api/room                 create (host picks the opening بيت, or a ديوان)
...
/**
 * Push the ENDING of each named room to whoever is still connected to it —
 * called after an account deletion settles the rooms its owner was live in
 * (server/users.ts `deleteUserAccount`). A room the deleted account HOSTED is
 * already gone by now (it cascaded with him), so `findRoomByCode` returns null
 * and it is simply skipped — exactly what a spectator's next poll would see. A
 * room he only played in survives, and its opponent gets «انتهت المساجلة» live
 * instead of waiting for a clock that will never move.
 */
export function notifyRoomsEnded(deps: RoomDeps, codes: readonly string[]): void {
  for (const code of codes) {
    const room = findRoomByCode(deps.users, code)
    if (room) broadcastRoom(deps, room, (state) => ({ type: "end", state }))
  }
}
...
export function roomRoutes(deps: RoomDeps): Hono {
  ...
  app.post("/", async (c) => {
    ...
    //
    // The shelf is opened under the SAME gate `/api/albums/:code` uses
    // (`readableAlbum`), so a ديوان the host cannot read is «لا ديوان بهذا
    // الرمز» here exactly as it is there — a room must not become a way to
    // learn that a private shelf exists. Then the floor: below ten playable
    // أبيات the chain concedes on its first turn, which is not a contest.
    let albumId: number | null = null
    let pool: number[] | undefined
    if (body.albumCode !== undefined) {
      const album = readableAlbum(deps.users, body.albumCode, user.id)
      if (album === null) return fail(c, "album_not_found")
      const resolved = albumPool(deps.db, deps.users, album)
      if (resolved.baitIds.length < ALBUM_LIMITS.playableFloor) return fail(c, "album_too_thin")
      albumId = album.id
      pool = resolved.baitIds
    }

    let row = null
    if (body.baitId !== undefined) {
      // Inside a ديوان the مطلع must be ON the shelf: a room whose opening بيت
      // its own rule would refuse is a room the guest cannot answer.
      if (pool !== undefined && !pool.includes(body.baitId)) return fail(c, "bad_bait")
      ...
      row = randomStartingBait(deps.db, [], pool)
    ...
    upgradeWebSocket((c) => {
      ...
      return {
        onOpen(_evt, ws) {
          ...
          //
          // ONE exception, and it is bounded to the case that needs it: a
          // non-seated arrival on a room that is still WAITING is the invited
          // guest reaching the lobby, and the host has no other way to learn
          // that his link was opened (`atDoor` on his snapshot). A waiting room
          // has the host and this arrival on it and nothing else, so the fanout
          // the cap was protecting cannot happen here — the pathological case
          // was N spectators on a LIVE مساجلة.
          if (seat !== null || live.status === "waiting") {

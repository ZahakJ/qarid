...
describe("POST /api/game/verify — rejections", () => {
  ...
  it("never suggests a بيت that starts on the wrong letter — a door into a wall", async () => {
    /**
     * The «هل تقصد؟» cards are TAPPABLE: pressing one fills the field and
     * re-submits it through this same verifier. Before the filter, a lost life
     * was answered with three suggestions on whatever letters the FTS ranker
     * happened to like — measured on the real corpus, a «ض» turn came back with
     * ا, ز and ل — and each one was refused a second later by the wrong-letter
     * check the fuzzy branch already enforces.
     */
    // The nonsense itself starts on ز, so the typed-text precheck lets it
    // through and the fuzzy ladder is what fails — which is the state the
    // audit measured.
    const required = "ز"
    const res = await verify({ text: "زرافة تأكل الموز في الحديقة الكبيرة جدا اليوم", requiredLetter: required })
    expect(res.ok).toBe(false)
    if (res.ok || res.reason !== "not_found") throw new Error(`expected not_found, got ${JSON.stringify(res)}`)
    // Either nothing survived (and the card says so) or every survivor is playable.
    for (const b of res.suggestions) expect(b.firstLetter).toBe(required)
  })
...
// ═════════════════════════════════════════════════════════════════════════════
// «مساجلة في ديوان فلان» — the شاعر «قيد» end to end
// ═════════════════════════════════════════════════════════════════════════════

/**
 * The شاعر filter is the one «قيد» `combo_counts` has no key for, so it exercises
 * a different half of the engine from عصر/بحر: the pool is counted by a live
 * grouped scan, the opening letter is drawn from THAT distribution rather than
 * from the corpus's, and فحل (`fame <= 2`) is empty by construction for every
 * famous ديوان and must relax rather than concede.
 *
 * Nothing here is hard-coded: the شاعر is whichever one the fixture files the
 * most game-playable أبيات under, and the two letters are read off his own
 * distribution — a fixture that gains a قصيدة must not turn this file red.
 */
describe("«القيود» with a شاعر", () => {
  /** slug of the fixture's fattest ديوان, and how its أبيات spread over letters */
  let scoped: { slug: string; name: string; total: number }
  /** a letter this شاعر opens أبيات on, and one the corpus has but he does not */
  let his: string
  let notHis: string
  ...
  beforeAll(() => {
    const row = db
      .q(
        `SELECT po.slug AS slug, po.name AS name, COUNT(*) AS n
         FROM game_baits gb JOIN poems p ON p.id = gb.poem_id JOIN poets po ON po.id = p.poet_id
         GROUP BY po.id ORDER BY n DESC, po.id ASC LIMIT 1`,
      )
      .get() as { slug: string; name: string; n: number }
    scoped = { slug: String(row.slug), name: String(row.name), total: Number(row.n) }

    const mine = new Set(
      (
        db
          .q(
            `SELECT DISTINCT gb.first_letter AS l
             FROM game_baits gb JOIN poems p ON p.id = gb.poem_id JOIN poets po ON po.id = p.poet_id
             WHERE po.slug = ?`,
          )
          .all(scoped.slug) as Array<{ l: string }>
      ).map((r) => String(r.l)),
    )
    const all = (
      db.q("SELECT gb.first_letter AS l, COUNT(*) AS n FROM game_baits gb GROUP BY 1 ORDER BY n DESC").all() as Array<{
        l: string
        n: number
      }>
    ).map((r) => String(r.l))
    his = all.find((l) => mine.has(l))!
    notHis = all.find((l) => !mine.has(l))!
    expect(his).toBeTruthy()
    expect(notHis).toBeTruthy()
  })

  // ── the count the setup screen shows ──────────────────────────────────────

  it("counts the pool live, because combo_counts has no شاعر key", async () => {
    const body = GamePoolResponseSchema.parse(
      (await get(`/api/game/pool?difficulty=brutal&poet=${encodeURIComponent(scoped.slug)}`)).body,
    )
    expect(body.total).toBe(scoped.total)
    // and the per-letter breakdown is the same query, letter by letter
    for (const { letter, count } of body.byLetter) {
      const n = Number(
        (
          db
            .q(
              `SELECT COUNT(*) AS n FROM game_baits gb JOIN poems p ON p.id = gb.poem_id
               JOIN poets po ON po.id = p.poet_id WHERE po.slug = ? AND gb.first_letter = ?`,
            )
            .get(scoped.slug, letter) as { n: number }
        ).n,
      )
      expect(count, letter).toBe(n)
    }
  })

  it("narrows further when a بحر is set beside the شاعر", async () => {
    const meter = db
      .q(
        `SELECT m.slug AS slug FROM game_baits gb JOIN poems p ON p.id = gb.poem_id
         JOIN poets po ON po.id = p.poet_id JOIN meters m ON m.id = gb.meter_id
         WHERE po.slug = ? GROUP BY m.id ORDER BY COUNT(*) DESC LIMIT 1`,
      )
      .get(scoped.slug) as { slug: string } | undefined
    if (meter === undefined) return
    const both = GamePoolResponseSchema.parse(
      (
        await get(
          `/api/game/pool?difficulty=brutal&poet=${encodeURIComponent(scoped.slug)}&meter=${encodeURIComponent(String(meter.slug))}`,
        )
      ).body,
    )
    expect(both.total).toBeGreaterThan(0)
    expect(both.total).toBeLessThanOrEqual(scoped.total)
  })

  // «فحل» is fame <= 2 and fame is a property of the شاعر, so a famous ديوان is
  // EXACTLY zero there — the case «العدد المتاح» would otherwise print as a bare
  // 0 over a duel that plays perfectly well out of the رتبة it relaxes to.
  it("reports an empty رتبة honestly, with the relaxed pool beside it", async () => {
    const fame = Number(
      (db.q("SELECT fame FROM poets WHERE slug = ?").get(scoped.slug) as { fame: number }).fame,
    )
    const body = GamePoolResponseSchema.parse(
      (await get(`/api/game/pool?difficulty=hard&poet=${encodeURIComponent(scoped.slug)}`)).body,
    )
    if (fame === 3) expect(body.total).toBe(0)
    expect(body.effectiveTotal).toBeGreaterThanOrEqual(body.total)
    expect(body.effectiveTotal).toBeGreaterThan(0)
  })

  it("answers zeros, not a 400, for a شاعر the artefact never heard of", async () => {
    const body = GamePoolResponseSchema.parse((await get("/api/game/pool?poet=la-ahad-bihatha-alism")).body)
    expect(body.total).toBe(0)
    expect(body.effectiveTotal).toBe(0)
    expect(body.byLetter).toHaveLength(28)
  })

  // ── the opening ───────────────────────────────────────────────────────────

  /** Who recited it — read off the corpus, never off the answer's own claim. */
  function poetOf(baitId: number): string {
    const row = db
      .q("SELECT po.slug AS slug FROM baits b JOIN poems p ON p.id = b.poem_id JOIN poets po ON po.id = p.poet_id WHERE b.id = ?")
      .get(baitId) as { slug: string } | undefined
    return row === undefined ? "" : String(row.slug)
  }

  it("opens out of his ديوان and nobody else's, on every seed", async () => {
    for (let i = 0; i < 12; i++) {
      const body = GameStartResponseSchema.parse(
        (await post("/api/game/start", { difficulty: "brutal", filters: { poet: scoped.slug }, seed: `p${i}` })).body,
      )
      expect(body.ok, `seed p${i}`).toBe(true)
      if (!body.ok) continue
      expect(poetOf(body.bait.id)).toBe(scoped.slug)
      expect(body.bait.poet.slug).toBe(scoped.slug)
    }
  })

  // The regression this test is really about: the opening letter used to be
  // drawn from the CORPUS's letter distribution even when the قيود named a
  // شاعر, so eight fat corpus letters could all be letters he never opens on
  // and the duel answered «لا يوجد بيت» over a ديوان that plainly had one.
  // Measured on data/qarid.db before the fix: 136 of 2,400 poet-scoped openings.
  it("never concedes an opening a small ديوان can answer", async () => {
    const small = db
      .q(
        `SELECT po.slug AS slug, COUNT(*) AS n
         FROM game_baits gb JOIN poems p ON p.id = gb.poem_id JOIN poets po ON po.id = p.poet_id
         GROUP BY po.id HAVING n BETWEEN 2 AND 8 ORDER BY po.id ASC LIMIT 6`,
      )
    ...
    for (const row of small) {
      for (let i = 0; i < 4; i++) {
        const picked = pickBait(db, {
          difficulty: "brutal",
          tailBias: "none",
          filters: { poet: String(row.slug) },
          seed: `small:${row.slug}:${i}`,
        })
        expect(picked, `${row.slug} (${row.n} أبيات), seed ${i}`).not.toBeNull()
        expect(poetOf(Number(picked!.row.b_id))).toBe(String(row.slug))
      }
    }
  })

  it("says no_bait for a شاعر who is not in the artefact", async () => {
    const body = GameStartResponseSchema.parse(
      (await post("/api/game/start", { filters: { poet: "la-ahad-bihatha-alism" } })).body,
    )
    expect(body.ok).toBe(false)
    if (body.ok) return
    expect(body.reason).toBe("no_bait")
  ...
  // ── the reply, and «خرج عن القيود» ────────────────────────────────────────

  it("replies out of his ديوان on a letter he answers", async () => {
    const body = GameReplyResponseSchema.parse(
      (await post("/api/game/reply", { letter: his, difficulty: "brutal", filters: { poet: scoped.slug } })).body,
    )
    expect(body.ok).toBe(true)
    if (!body.ok) return
  ...
  it("leaves the ديوان — and SAYS so — on a letter he never opens on", async () => {
    const body = GameReplyResponseSchema.parse(
      (await post("/api/game/reply", { letter: notHis, difficulty: "brutal", filters: { poet: scoped.slug } })).body,
    )
    expect(body.ok).toBe(true)
    if (!body.ok) return
    expect(body.relaxed).toBe(true)
    expect(poetOf(body.bait.id)).not.toBe(scoped.slug)
  })

  it("honours the transcript's exclusions inside the ديوان", async () => {
    const mine = (
      db
        .q(
          `SELECT gb.bait_id AS id FROM game_baits gb JOIN poems p ON p.id = gb.poem_id
           JOIN poets po ON po.id = p.poet_id WHERE po.slug = ? AND gb.first_letter = ?`,
        )
        .all(scoped.slug, his) as Array<{ id: number }>
    ).map((r) => Number(r.id))
    expect(mine.length).toBeGreaterThan(0)
    // Every بيت he could legally answer with is already in the transcript, so
    // the only honest answer is one from outside the قيود.
    const body = GameReplyResponseSchema.parse(
      (
        await post("/api/game/reply", {
          letter: his,
          difficulty: "brutal",
          filters: { poet: scoped.slug },
          excludeBaitIds: mine,
        })
      ).body,
    )
    ...
    if (!body.ok) return
    expect(body.relaxed).toBe(true)
    expect(mine).not.toContain(body.bait.id)
  })

  it("buys a hint out of the same ديوان", async () => {
    const body = GameHintResponseSchema.parse(
      (
        await post("/api/game/hint", {
          kind: "poet",
          letter: his,
          difficulty: "brutal",
          filters: { poet: scoped.slug },
    ...
    expect(body.ok).toBe(true)
    if (!body.ok || body.kind !== "poet") return
    expect(body.poet.slug).toBe(scoped.slug)
  })

  // ── the asymmetry ─────────────────────────────────────────────────────────

  // «تحصر ما يُنشده الخصم، لا ما تُجيب به أنت» is the whole of the القيود section
  // on the setup screen, and a شاعر filter is where breaking it would be fatal:
  // a duel «في ديوان المتنبي» that also demanded المتنبي's own أبيات as ANSWERS
  // would be unplayable by anyone alive. `verifyAnswer` never reads `filters`;
  // this is the test that says so out loud.
  it("still accepts an answer by any other شاعر", async () => {
    const res = await verify({ text: wardah.full, filters: { poet: scoped.slug } })
    expect(res.ok).toBe(true)
    if (!res.ok) return
    expect(res.bait.poet.slug).not.toBe(scoped.slug)
  })
})

...
describe("bearer issuance is confined to a secure origin", () => {
  it("is allowed on https and on localhost, refused over plain http to a public host", async () => {
    ...
    const https = createApp(configFor({ PUBLIC_ORIGIN: "https://qarid.avicenna.space" }), null, httpsDb).app
    ...
    const insecure = createApp(configFor({ PUBLIC_ORIGIN: "http://qarid.avicenna.space" }), null, insecureDb).app
...
describe("CORS allowlist for the Capacitor app", () => {
  ...
  beforeAll(async () => {
    ...
    app = createApp(configFor({ PUBLIC_ORIGIN: "https://qarid.avicenna.space" }), null, db).app
  ...
  it("still 403s every OTHER cross-origin write — the CSRF fix is not weakened", async () => {
    for (const origin of ["https://meme.avicenna.space", "https://evil.example", "https://localhost.avicenna.space"]) {
      const res = await app.request("/api/profile/update", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  it("does not answer a preflight for a non-allowlisted origin", async () => {
    const res = await app.request("/api/auth/login", { method: "OPTIONS", headers: { Origin: "https://meme.avicenna.space" } })
    expect(res.headers.get("Access-Control-Allow-Origin")).toBeNull()

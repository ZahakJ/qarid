 ...
 * `https://qarid.avicenna.space` is CROSS-ORIGIN and carries
 * `Sec-Fetch-Site: cross-site` and an `Origin` header — exactly what the CSRF
 * guard below refuses for a sibling `*.avicenna.space` page. These three exact
 ...
 * `avicenna.space` — so `SameSite=Lax` never sends `qarid_sess` to us from it

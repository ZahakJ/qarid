 ...
 * from a sibling `*.example.com` page renamed a signed-in reader's account,
...
const PUBLIC_ORIGIN = "https://qarid.example.com"
...
describe("originAllowed / fetchSiteAllowed", () => {
  ...
  it("refuses a SIBLING under the same registrable domain — the whole point", () => {
    // SameSite=Lax is computed on `example.com`, so every one of these sends
    // the session cookie with a forged POST.
    for (const sibling of [
      "https://meme.example.com",
      "https://alchemy.example.com",
      "https://vestige.example.com",
    ]) {
...
describe("the forged cross-site POST", () => {
  it("refuses a rename from a sibling subdomain, cookie and all", async () => {
    const res = await post(
      "/api/profile/update",
      { displayName: "مخترق" },
      { Origin: "https://meme.example.com", "Sec-Fetch-Site": "same-site" },
    )
  ...
  it("refuses a forged logout, which takes no body at all", async () => {
    const res = await app.request("/api/auth/logout", {
      method: "POST",
      headers: { Host: HOST, Cookie: `qarid_sess=${token}`, Origin: "https://meme.example.com" },
    })
    expect(res.status).toBe(403)
...
describe("GET /api/profile/:username", () => {
  it("does not publish room codes to a reader who is not the account", async () => {
    ...
    // A رجعة: still `waiting`, but its guest is already named. A waiting room
    // with NO guest is one that was opened and never entered, and the profile
    // deliberately keeps those out of «آخر المساجلات» (server/users.ts
    // `recentMatches`) — so the room this security property is measured on has
    // to be one the list actually carries.
    const rival = createUser(users, { username: "rivalpoet", displayName: "خصم", passHash: "scrypt$y", now })!
    users.raw
      .prepare(
        `INSERT INTO rooms (code, host_user_id, guest_user_id, starting_bait_id, mode, timer_s, strikes, status,
         ...
         VALUES ('BADIRU', ?, ?, 1, 'rhyme', 30, 3, 'waiting', ?, ?, 'abcdefghijkmnpqrstuvwxyz')`,
      )
      .run(alice.id, rival.id, now, now)

...
describe("duel statistics over the §5 tables", () => {
  ...
  it("leaves out a room that was OPENED and never entered", () => {
    /**
     * The profile's tiles count only decided rooms, so a list that carried
     * every room ever opened disagreed with the number above it on the same
     * screen — «2 مساجلة» over «لم تُحسم · بانتظار خصم» three times, each
     * linking to a dead waiting room. A waiting room with a guest (a رجعة) is
     * a real match and stays.
     */
    const now = Date.now()
    const me = createUser(db, { username: "sharifradi", displayName: "الشريف الرضي", passHash: "x", now })!
    const foe = createUser(db, { username: "farazdaq", displayName: "الفرزدق", passHash: "x", now })!
    const insert = (code: string, guest: number | null, status: string) =>
      db.raw
        .prepare(
          `INSERT INTO rooms (code, host_user_id, guest_user_id, starting_bait_id, mode, status, created_at, updated_at)
           VALUES (?, ?, ?, 1, 'rhyme', ?, ?, ?)`,
        )
        .run(code, me.id, guest, status, now, now)

    insert("NEVERR", null, "waiting")
    insert("REMATC", foe.id, "waiting")
    insert("LIVEEE", foe.id, "active")

    const codes = recentMatches(db, me.id, 10).map((r) => r.code).sort()
    expect(codes).toEqual(["LIVEEE", "REMATC"])
  })
})
...
describe("cookie Secure flag", () => {
  ...
  it("is set behind an https PUBLIC_ORIGIN and absent on a plain-http dev box", async () => {
    const https = createApp(configFor({ PUBLIC_ORIGIN: "https://qarid.example.com" }), null, db).app

...
/**
 * THE ANCHOR a ديوان keeps instead of an id — `baits.h_full` as a decimal
 * string, computed from the بيت's own text.
 *
 * `public_id` is `q<row id>` for 73 % of قصائد and every id in the artefact
 * MOVES on a rebuild (CLAUDE.md §The artefact), so a table of ids in the
 * writable database rots at the next `npm run ingest`. Content does not: this
 * is exactly the hash `scripts/ingest/transform.ts` writes into `baits.h_full`
 * — one normalizer, one hash, spelled once — and `baits_hfull` indexes it, so
 * re-finding a بيت after a rebuild is a point lookup.
 *
 * It is a STRING because JSON has no 64-bit integer and because the users
 * database stores it in a TEXT column; the corpus side converts back with
 * `BigInt(...)` at the one place it queries. Null for a بيت the scrape left
 * without a عجز: `transform.ts` writes no `h_full` for those either, so there
 * is nothing to anchor to and «أضِف إلى ديوان» does not offer itself.
 */
export function baitAnchor(sadr: string, ajuz: string | null | undefined): string | null {
  const half = ajuz === null || ajuz === undefined || ajuz.trim() === "" ? null : ajuz
  if (half === null) return null
  const norm = normalizeArabic(`${sadr} ${half}`)
  if (norm === "") return null

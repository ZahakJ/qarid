...
/**
 * «بلا عنوان» is what the ingest writes when the source had no title, and the
 * value `headingOf` tests against to decide whether a قصيدة is named by its
 * عنوان or by its مطلع. It sits here rather than in the client because the
 * SHARE head (`GET /p/:id`, server/share.ts) has to name a قصيدة exactly the
 * way its own page does — two spellings of this string is two different names
 * for the same قصيدة, one in the tab and one in the unfurl.
 */

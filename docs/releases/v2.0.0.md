...
- **على الوِب**: <https://qarid.avicenna.space> — لا شيءَ يُنصَّب.
...
that same built shell: app id `space.avicenna.qarid`, bearer-token sessions for a
...
- **Web** — <https://qarid.avicenna.space>. Nothing to install.
...
- **Android** — download the `.apk` from this release's assets and allow
  installation from an unknown source. 4.5 MB, app id `space.avicenna.qarid`,
  `minSdk 24` (Android 7.0). **Verify the signature before you install it** —
  `apksigner verify --print-certs <the file>` must print:

  ```
  SHA-256: 8C:05:1E:14:92:22:E1:4F:9A:B2:2D:79:16:4D:CC:32:9C:73:4B:EA:65:62:9B:A6:75:43:F6:B9:2F:CB:EA:E5
  ```

...
Owner ask: "a native super cool phone version, at least apk/android, so I can play and share with friends and siblings." Do NOT start until the v2 close-out (hardening + live verification + GitHub release) has fully landed.
...
Environment verified on this box: full Android SDK at ~/Android/Sdk (build-tools, platforms, cmdline-tools, NDK), JDK 21 at ~/Android/jdk-21.0.11+10, prior signing experience (concord-release.keystore — do NOT reuse; generate qarid-release.keystore, backed up outside the repo).
- Capacitor app id space.avicenna.qarid, bundled web assets (fast start, offline shell) with API base https://qarid.avicenna.space.
...
- Native touches: share sheet (share cards + daily text via @capacitor/share), haptics on duel events (accept/reject/timeout), status-bar + nav-bar inked to theme, hardware back = router back, splash (gold nib on ink), app icon, keep-awake during a duel turn, deep links qarid.avicenna.space/#/room/<code> open the app (assetlinks.json served by Hono).

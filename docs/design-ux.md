...
```css
.bayt { display:grid; grid-template-columns: 1fr var(--gutter,3.25rem) 1fr; align-items:baseline; }
.bayt .sadr { text-align:start } .bayt .ajuz { text-align:end }
```
DOM order sadr, gutter, ajuz → under RTL both hemistichs pushed to outer margins with quiet centre channel. Gutter empty; on hover/focus-within a 12px gold Shamsa SVG fades in 45%. Bayt number in 4ch inline-start column, text-3, Arabic-Indic. <860px: single column, `.ajuz` padding-inline-start 1.75rem + 1px line-1 inline-start border.
